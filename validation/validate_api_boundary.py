...
com.cobbletowers.api is what an addon compiles against. If a public signature there names a
CobbleTowers internal type, a Cobblemon type or a CobbleRaids type, every consumer is coupled to it
whether it means to be or not. So this reads the compiled classes -- not the source -- and fails on
any public or protected field, method, super type or generic signature that references a type
outside:

    java/                    the JDK
    net/minecraft/           Minecraft, which every consumer already depends on
    com/cobbletowers/api/    the API itself

Reading bytecode means it cannot be fooled by a fully qualified name, an import alias or a type
reached through a generic argument, and it keeps working when classes are renamed or added. It is
the companion of validate_architecture.py, which keeps CobbleRaids internals out of the whole mod.

    python validation/validate_api_boundary.py                          # compiled classes
    python validation/validate_api_boundary.py build/libs/CobbleTowers-x.jar
"""
...
API_PREFIX = "com/cobbletowers/api/"
